...
Desde el 29/08/2026 usa el MISMO lenguaje visual que la imagen única del
post (`generate_single_post_image.py` / AUTOMATION_BRIEF.md sección 3):
fondo crema #FFFCF4, texto tinta #111111, League Spartan (ExtraBold para
titulares, Bold para cuerpo/etiquetas), resaltado naranja #FF914D tipo
"subrayador" en las frases clave, foto+logo arriba en el mismo sitio en
toda página — para que el recurso descargable y el post se sientan la
misma pieza de marca, no dos estilos distintos (antes, 24-28/08/2026,
usaba el lenguaje oscuro del carrusel — fondo negro con rejilla sutil,
texto crema, ArchivoBlack/Barlow-Bold; se retiró porque el post ya no usa
ese formato desde el 28/08).
...
BG = colors.HexColor("#FFFCF4")     # crema
BORDER = colors.HexColor("#E8E1CE")  # borde sutil de tarjeta sobre el crema
ORANGE = colors.HexColor("#FF914D")  # titular, resaltado
INK = colors.HexColor("#111111")     # texto principal
GRAY = colors.HexColor("#79746A")    # subtexto (pie de página, kicker) — mas oscuro que en el post porque aquí va sobre crema, no sobre la banda negra
...
# El logo (assets/branding/logo.png) es texto negro + naranja sobre fondo
# transparente — sobre el crema del PDF se lee bien tal cual, sin recolorear
# (el truco de recolorear a crema solo hacía falta con el fondo negro del
# carrusel, retirado el 29/08/2026).
...
style_h1_cover = style("h1_cover", fontName="LeagueSpartanExtraBold", fontSize=27, leading=35, spaceAfter=14)
style_sub_cover = style("sub_cover", fontSize=12.5, leading=18)
style_author_name = style("author_name", fontName="LeagueSpartanExtraBold", fontSize=12, leading=15, textColor=INK)
style_author_role = style("author_role", fontSize=9, leading=12, textColor=INK)
style_h2 = style("h2", fontName="LeagueSpartanExtraBold", fontSize=18, leading=23, spaceAfter=10)
style_body = style("body", fontSize=10, leading=15.5, spaceAfter=8)
style_intro = style("intro", fontSize=11.5, leading=18)
style_error_title = style("error_title", fontName="LeagueSpartanExtraBold", fontSize=13, leading=17)
style_label = style("label", fontSize=8.5, leading=12, textColor=ORANGE, spaceBefore=10, spaceAfter=3)
style_cta_title = style("cta_title", fontName="LeagueSpartanExtraBold", fontSize=22, leading=28, spaceAfter=14)
style_cta_body = style("cta_body", fontSize=11, leading=16)
style_cta_phase_h = style("cta_phase_h", fontName="LeagueSpartanExtraBold", fontSize=11.5, leading=15, textColor=INK)
...
style_intro_stat_num = style("intro_stat_num", fontName="LeagueSpartanExtraBold", fontSize=48, leading=48,
...
def _header(c, doc):
    ...
    naranja arriba a la derecha — mismo patrón que la firma de la imagen
    del post, y en el mismo sitio en TODAS las páginas."""
...
def page_chrome(c: pdfcanvas.Canvas, doc):
    """Fondo crema liso — sin rejilla (esa era la estética del carrusel
    oscuro, retirada el 29/08/2026 para que el PDF luzca tan minimalista
    como la imagen del post)."""
    c.saveState()
    c.setFillColor(BG)
...
def badge(text, size=9 * mm, bg=ORANGE, fg=INK, font="LeagueSpartanExtraBold", fontsize=12):
